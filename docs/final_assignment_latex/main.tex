...
从最终方案看，我们以 G+P 稳定主干为锚点，引入 OpenFace 面部动态形成 Young 多模态融合方案；语音线没有进入主提交，但作为课程相关探索和失败分析写入报告。这一取舍来自实验结果，而不是忽略语音模态。
...
最终提交名为 \code{young\_final\_t4p25\_t2p11}。从模态使用上看，它以 IMU 步态与 Big Five 人格作为稳定主干，并引入 OpenFace 面部动态特征进行融合；语音模态作为系统探索与负面结果保留。严格表述时，它是一条围绕 MPDD-Young 的行为动态多模态融合方案，而不是把所有官方模态无差别堆叠的方案。
...
OpenFace 原始每帧 710 维，第一列为帧号，去掉后仍有 709 维。我们只选择其中 43 列：gaze 8 维、AU intensity 17 维、AU presence 18 维。选择依据有两层：第一，这些列和面部运动学最直接相关，能表达注视方向、动作单元强度和动作单元是否出现；第二，它们可以进一步压缩为 mean、std、帧间速度、频谱和跨事件差等低维动态统计，避免在 88 人样本上直接使用全量高维序列。

\subsection{二分类与三分类标签的区别}

二分类只判断 normal/depressed，训练集基本平衡；三分类进一步区分 normal/mild/severe，其中 severe 只有 10 人，因此更难稳定。二分类和三分类不是简单重复：一个样本的 \phq 排序合理，不代表它一定落在正确类别边界上。最终分类头使用融合 \phq 与 ORIG 主干双确认，本质上就是针对二/三分类边界单独优化，而不是完全依赖连续 \phq 阈值。
...
下表为根据现有 PPT 与技术材料整理的分工版本，具体姓名与职责可按小组最终确认结果微调。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{l l X}
    ...
    成员 & 分工方向 & 具体工作 \\
    \midrule
    张子路 & 主干模型与复现 & 负责 IMU + Big Five 主干模型复现，整理 ORIG 有序回归流程，核对最终复现代码包与提交结果。 \\
    郭子逸 & 面部特征工程 & 负责 OpenFace gaze/AU 特征筛选，构造 auvel、e3only、fatigue、auspec 等面部动态特征。 \\
    龚正硕 & 语音模态探索 & 负责 openSMILE、MFCC、wav2vec2、GRU + attention、ASR + \phq 语义证据等语音实验。 \\
    丁一鸣 & 融合方案与汇报 & 负责多路融合、\ccc 校准、分类头规则整理，以及汇报 PPT 和最终文档统筹。 \\
    蒋一鸣 & 实验对比与结果整理 & 负责阶段性提交记录、融合权重与阈值对比、消融结果整理和问题分析。 \\
    谈亮 & 工程整理与交付 & 负责 clean 版本代码整理、依赖与运行说明检查、最终提交包结构核对和可复现性说明。 \\
    \bottomrule
  \end{tabularx}
\end{table}
