...
小组分工按技术模块组织，而不是按单纯的文档、复现或提交工作拆分。每位成员都负责一组具体的特征、模型或融合探索，并完整参与该方向的方案设计、代码实现、实验分析和交付表述；最终提交前再统一进行数据核对、结果复盘和材料检查。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{1.35cm} >{\raggedright\arraybackslash}p{3.05cm} X}
    \toprule
    成员 & 牵头技术方向 & 技术与交付贡献 \\
    \midrule
    张子路 & IMU + Big Five 主干、ORIG 有序回归 & 负责 IMU 统计特征与 Big Five 心理量表特征融合，调试 ORIG 有序回归主干；参与 \ccc 回归指标、二分类阈值和三分类区间的联动分析，并将主干模型与复现实验写入交付材料。 \\
    郭子逸 & OpenFace 语义映射、gaze/AU 面部特征 & 负责查阅面部行为与抑郁症先验，构建 OpenFace 维度--面部位置对应表，借助 LLM 筛选 gaze、AU、pose 等候选维度；实现 auvel/e3only 中的面部动态 ranker，并参与融合验证。 \\
    龚正硕 & 音频低层特征、预训练语音表征 & 负责 openSMILE、MFCC、wav2vec2 等语音特征管线，尝试 GRU + attention 与语音回归/分类头；分析语音模态在本地验证和 hidden test 上的偏差，整理其未进入最终融合的原因。 \\
    丁一鸣 & ASR 语义证据、\phq 文本线、分类规则 & 负责 ASR 转写质量检查、\phq 关键词/语义证据设计与 e2speech 探索；参与把语音语义证据同 ORIG 分数、面部 ranker 对齐，并牵头 AND 二分类规则和三分类细分规则整理。 \\
    蒋一鸣 & 多路融合、分布校准、消融对比 & 负责 ORIG、auvel、e3only、fatigue、auspec 等候选分支的分数融合、\ccc 分布校准、权重搜索与阈值消融；跟踪 leaderboard 与本地验证差异，整理结果表和问题分析。 \\
    谈亮 & fatigue/auspec 面部动态特征、工程化管线 & 负责 fatigue、auspec 两类面部动态统计特征构造与 ranker 训练脚本，复查各分支输入维度、特征缓存和依赖版本；共同完成 run pipeline、clean 代码包与 PDF/PPT 表述一致性检查。 \\
    \bottomrule
  \end{tabularx}
\end{table}
